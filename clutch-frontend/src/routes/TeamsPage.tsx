...
export const TeamsPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return (teams ?? [])
      ...
      .sort(
        (a, b) =>
          Number(favTeams.includes(b.id)) - Number(favTeams.includes(a.id)) ||
    ...
    <Page>
      <div className="px-5 pt-4 pb-3">
        ...
        <p className="mt-1.5 text-[13px] leading-none font-medium text-dim">
          Filtre par jeu et suis tes équipes
        </p>
      </div>
        ...
        <div className="mb-4 flex items-center gap-2.5 rounded-[14px] bg-surface-2 px-3.5 py-3">
          ...
          {q && (
            <button
              onClick={() => setQ("")}
              aria-label="Effacer"
              className="grid cursor-pointer text-dim"
            >
        ...
        {filtered.length === 0 && !(teamsPending || matchesPending) ? (
          ...
          filtered.map((team) => {
            ...
            return (
              <div
                key={team.id}
                className="flex items-center gap-3 rounded-3xl border border-line-2 bg-surface p-3.5"
              >
                <Link
                  to={`/team/${team.id}`}
